...
일반적으로 Old 영역의 크기는 Young 영역의 두 배정도여서 GC 시간이 더 많이 거린다.
...
Parallel GC는 Serial GC와 기본적인 알고리즘은 같다. 그러나 Serial GCsms GC를 처리하는 스레드가 하나인 것에 비해, Parallel GC는 GC를 처리하는 쓰레드가 여러 개이다. 그렇기 때문에 Serail GC보다 빠르게 객체를 처리할 수 있다.
...
G1 GC를 이해하기 위해선 HotSpot VM에서 사용하는 전통적인 방식인 통으로 되어이 있는 자바 Heap 메모리를 Young 영역과 Old 영역으로 나누는 개념을 잊어야한다.
